...
# df_stns_all[0].split('19701001')[0].split('_')
stns = [int(df.split('19701001')[0].split('_')[-2]) for df in df_stns_all]
...
for _var in variables:
    ...
    hdf5_path = os.path.join(r'X:\staff\elhachem\2023_09_01_ViTaMins\Data',
                             'CAMELS_GB_%s_%s_%s_%s.h5'
                             % (in_freq, '1970', '2015', _var))

    

    if True:#not os.path.isfile(hdf5_path):
        ...
        # timestamps
        hf.create_group(where=hf.root,  name='timestamps',
                        title='Timestamps of respective Aggregation as Start Time')
    
        hf.create_carray(where=hf.root.timestamps, name='isoformat',                         atom=tables.StringAtom(19), shape=(nts_max,),
                         chunkshape=(10000,), title='Strings of Timestamps in Isoformat')
    ...
    for i_idx, df_stn in enumerate(df_stns_all):
        # break
        ...
        if len(str(stn)) > 0:
            ...
            df_in_var = df_in.loc[:, _var]
    
            if df_in_var.size > 0:
    
                blank_df = pd.DataFrame(index=dates, data=np.full(shape=dates.shape, fill_value=np.nan),
                                        columns=[stn])
                # print(df_in_var)
                print('Dataframe has %d data ' %
                      df_in_var.size, df_in_var.sum())
                # print(df_in_var.index[0], df_in_var.index[-1])
                try:
                    cmn_idx = blank_df.index.intersection(df_in_var.index)
                    # index.intersection(df_in.index).fillna(-9)
                    blank_df.loc[cmn_idx, stn] = df_in_var.values
                ...
                df_in = pd.DataFrame(index=blank_df.index,
                                     data=np.full(shape=blank_df.index.shape, fill_value=-9))
            try:
                start_idx = df_in.index[0]
            except Exception as msg:
                print('Error with start index', msg)
            end_idx = df_in.index[-1]
            hf.root.coord.lon[i_idx] = df_coords['gauge_lon'].loc[stn]
            hf.root.coord.lat[i_idx] = df_coords['gauge_lat'].loc[stn]
            # df_coords['Stationshoehe'].loc[stn]
            # hf.root.coord.z[i_idx] = df_coords['gauge_elev'].loc[stn]
            ...
            hf.root.name[i_idx] = np.string_(str(stn))
