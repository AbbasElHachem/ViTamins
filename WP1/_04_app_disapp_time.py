...
def main():
    ...
    #     % radar_loc)
    dwd_hdf5 = HDF5(infile=path_to_data)
    dwd_ids = dwd_hdf5.get_all_names()

    n_vecs = int(1e4)

    data_freq_list = ['1440min']
    # in_refr_var_file = (
    #     r"X:\staff\elhachem\ClimXtreme\03_data\00_DWD"
    ...
    for _dwd_id in dwd_ids:
        ...
        # break
            df_stn = dwd_hdf5.get_pandas_dataframe_between_dates(
                _dwd_id, event_start=beg_date, event_end=end_date).dropna()
            ...
            #         time_idx = np.arange(0, tot_refr_var_arr.shape[0]).astype(np.int64)
                    ...
                    depths2 = depth_ftn_mp(tot_refr_var_arr, tot_test_var_arr,
                                           usph_vecs, n_cpus, 1)
                    ...
                    # # plt.show()
                    # plt.savefig(
                    #     out_save_dir / (r'_%s.png' % (_dwd_id)), bbox_inches='tight')
                    # # depths2.shape
                    # plt.close()
                    ...
                    ad_ans.set_analysis_parameters(time_win_type, ws, analysis_style, peel_depth,
                        n_cpus, time_unit_step_size)
